...
it('renders power, efficiency, shares and pool when status exists', async () => {
  ...
  await render(<MinerSnapshotCard miner={miner} />);
  expect(screen.getByText('snapshot.power')).toBeTruthy();
  expect(screen.getByText('snapshot.efficiency')).toBeTruthy();
  expect(screen.getByText('snapshot.shares')).toBeTruthy();
...
it('shows N/A for pool when it is empty', async () => {
  ...
  await render(<MinerSnapshotCard miner={miner} />);
  expect(screen.getByText('snapshot.na')).toBeTruthy();
});
...
it('renders generated by HashWatch footer', async () => {
  const miner = makeMiner();
  await render(<MinerSnapshotCard miner={miner} />);
  expect(screen.getByText('snapshot.generatedBy')).toBeTruthy();
});
